...
def ObstacleSpace(obsXDIM, obsYDIM):
    obs = []
    for i in range(obsXDIM*100):
        obs.append(Node(obstacleStartX+i*0.01, obstacleStartY))
        obs.append(Node(obstacleStartX+i*0.01, obstacleEndY))
    for j in range(obsYDIM*100):
        obs.append(Node(obstacleStartX, obstacleStartY+j*0.01))
    return obs
